...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Calculate y(output), same as in the loss function
  ...
  delta_output = class_output - data.targets;
  delta_hidden = (model.hid_to_class' * delta_output) .* (hid_output .* (1 - hid_output));
  
  ret.input_to_hid = (1 / m) .* (delta_hidden * data.inputs') + wd_coefficient .* model.input_to_hid;
  ret.hid_to_class = (1 / m) .* (delta_output * hid_output') + wd_coefficient .* model.hid_to_class;
end
